...
def main() -> None:
    ...
    add_heading_cn(doc, "二、StaffDeck 核心功能", 1)
    add_table(
        doc,
        ["能力域", "功能要点"],
        [
    ...
    add_heading_cn(doc, "三、与 AIWork-OS 对照结论", 1)
    add_para(
        doc,
    ...
    add_heading_cn(doc, "四、一期详细设计（先做）", 1)
    add_para(doc, "一期只做“壳”，不改内核执行逻辑。", bold=True)
    add_para(
        doc,
    ...
    add_para(doc, "验收：创建绑定、发布可见、权限隔离、启用进 Chat、API 单测通过。")

    add_heading_cn(doc, "五、二期详细设计：SOP 状态机", 1)
    add_para(
        doc,
    ...
    add_heading_cn(doc, "七、质量、合规与里程碑", 1)
    add_bullets(
        doc,
        [
    ...
    add_heading_cn(doc, "九、立项后 3 天行动项", 1)
    add_bullets(
        doc,
        [
            "确认一期画廊字段与信息架构",
    ...
    out_md_docx = ROOT / "docs" / "digital-employee" / "StaffDeck能力融合-详细开发文档.docx"
    desk = Path.home() / "Desktop" / "StaffDeck能力融合-详细开发文档.docx"
    doc.save(str(out_md_docx := out_md_docx if False else out_md_docx := out1))
    # rewrite save cleanly
    doc.save(str(out1))
    doc.save(str(out2))
    print("Saved:")
    print(out1)
