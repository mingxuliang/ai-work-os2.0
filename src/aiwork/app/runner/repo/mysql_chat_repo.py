...
class MySQLChatRepository:
    ...
    async def load(self) -> Any:
        from aiwork.app.chats.models import ChatsFile
...
def patch_qwenpaw_chat_factory() -> bool:
    """Monkey-patch create_chat_service to use MySQLChatRepository."""
    try:
        import aiwork.app.workspace.service_factories as factories
    except Exception as exc:  # noqa: BLE001
        logger.warning("Cannot import service_factories: %s", exc)
        return False
    ...
    logger.info("Patched create_chat_service → MySQLChatRepository")
    return True
