...
normalized :class:`~aiwork.cli.tui.transport.base.TuiTransport` interface and
the :mod:`~aiwork.cli.tui.events` event union, so the transport is a swappable
seam: a future in-process transport can replace the ACP subprocess without
touching any widget.

Entry points:

* bare ``qwenpaw`` (no subcommand) -> :func:`~aiwork.cli.tui.launch.run_tui`
