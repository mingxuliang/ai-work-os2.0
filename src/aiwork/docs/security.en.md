...
**Default protection**: The `{WORKING_DIR}.secret/` directory (which stores API keys, authentication credentials, and provider configurations) is included in the sensitive-file list by default. By default, `WORKING_DIR` is `~/.aiwork/`, making the full path `~/.aiwork.secret/`.
...
```json
{
  "security": {
    "file_guard": {
      "enabled": true,
      "sensitive_files": ["~/.ssh/", "/etc/passwd", "~/.aiwork.secret/"]
    }
  }
}
```
...
Check the governance log (`aiwork.log`) for lines containing:
...
The scanner uses YAML rule files in `src/aiwork/security/skill_scanner/rules/signatures/`. You can customize the scan policy via a YAML policy file:

```python
from aiwork.security.skill_scanner import SkillScanner
from aiwork.security.skill_scanner.scan_policy import ScanPolicy
...
rm ~/.aiwork.secret/auth.json  # or $WORKING_DIR.secret/auth.json
