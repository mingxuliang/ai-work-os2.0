...
**Note**: After deletion, the workspace directory is retained (to prevent accidental data loss). To completely remove it, manually delete the `~/.aiwork/workspaces/{agent_id}` directory.
...
```
~/.aiwork/workspaces/{agent_id}/PROFILE.md
```
...
# Workspace: ~/.aiwork/workspaces/code_reviewer
...
Workspace: ~/.aiwork/workspaces/writer_bot
Profile: [Auto-generated detailed capability description]
```
...
cp -r ~/.aiwork/workspaces/abc123 ~/backups/agent-abc123-$(date +%Y%m%d)

# Backup all agents
cp -r ~/.aiwork/workspaces ~/backups/workspaces-$(date +%Y%m%d)
```
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Coding Mode ON + project_dir is git repo | Creates a **git worktree** under `<project_dir>/.aiwork/worktrees/`. Subagent works in the isolated worktree.   |
| Coding Mode OFF + workspace is git repo  | Creates a **git worktree** under `<workspace_dir>/.aiwork/worktrees/`. Subagent works in the isolated worktree. |
...
```bash
git worktree list
git worktree remove .aiwork/worktrees/<id>
```
