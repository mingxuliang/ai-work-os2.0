...
| QwenPaw `/proactive` | `src/aiwork/agents/memory/proactive`                        | `/proactive [minutes                                                                                        | on                                                                                     | off]` idle loop | A proactive chat request sent through `/api/console/chat` |
...
The embedded ReMe config comes from `src/aiwork/agents/memory/reme_config.py` and the user-facing defaults come from `ReMeLightMemoryConfig`.
...
QwenPaw's current `/proactive` command is implemented under `src/aiwork/agents/memory/proactive`. It is separate from ReMe's `proactive` job.
