...
For `requires` metadata, the parser checks keys in order: `metadata.openclaw.requires` → `metadata.aiwork.requires` → `metadata.requires`. The first one found is used.
