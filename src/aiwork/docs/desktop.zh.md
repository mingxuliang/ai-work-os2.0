...
- 应用会在后台运行，打开应用窗口
- 应用日志输出到：`~/Library/Logs/io.agentscope.aiwork.desktop/qwenpaw-desktop.log`
- 后端 sidecar 日志位于工作目录：`~/.aiwork/desktop.log`
...
1. 检查 `~/.aiwork/desktop.log` 文件查看错误
2. 使用上述终端命令启动，查看实时输出
...
- **日志位置**:
  - Windows: Debug 快捷方式终端查看；应用日志 `%LOCALAPPDATA%\io.agentscope.aiwork.desktop\logs\qwenpaw-desktop.log`；后端 `%USERPROFILE%\.qwenpaw\desktop.log`
  - macOS: 应用日志 `~/Library/Logs/io.agentscope.aiwork.desktop/qwenpaw-desktop.log`；后端 `~/.aiwork/desktop.log`
