...
- **手动编辑 `agent.json`** — 在智能体工作区的 `agent.json` 中（如 `~/.aiwork/workspaces/default/agent.json`），将需要的频道设 `enabled: true` 并填好鉴权信息；保存后自动重载，无需重启。
...
可以在console前端配置，或者修改智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）。
...
在智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）里找到 `channels.dingtalk`，填入对应信息：
...
在智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）中找到`channels.feishu`，只需填 **App ID** 和 **App Secret**（在开放平台「凭证与基础信息」里复制）：
...
4. 填写 iMessage 数据库路径。默认路径为 `~/Library/Messages/chat.db`，若你改过系统路径，请填实际路径。有以下两种填写方案：
   ...
   - 填写智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）：
...
在智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）里找到 `channels.discord`，填入对应信息：
...
在智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）里找到 `channels.qq`，把上面两个值分别填进 `app_id` 和 `client_secret`：
...
**方法二**在 `agent.json` 填写（如 `~/.aiwork/workspaces/default/agent.json`）
...
| `media_dir`              | string | `~/.aiwork/media` | 媒体文件（图片、文件等）下载目录          |
| `max_reconnect_attempts` | int    | `-1`               | WebSocket 最大重连次数（`-1` = 无限重连） |
...
- **登录方式**：首次使用时扫描二维码授权，Token 自动持久化到本地文件（默认 `~/.aiwork/wechat_bot_token`），后续启动无需重复扫码。
...
也可直接在智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）中配置：
...
| `bot_token_file` | string | `~/.aiwork/wechat_bot_token` | Token 持久化路径，下次启动自动读取                  |
| `base_url`       | string | 官方默认地址                  | iLink API 地址，一般留空使用默认值                  |
| `media_dir`      | string | `~/.aiwork/media`            | 接收到的图片、文件保存目录                          |
...
在智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）里找到 `channels.telegram`，填入对应信息：
...
3. 在控制台或智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）中配置 **URL** 和 **Token**。
...
在 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）中找到 `channels.matrix`：
...
在智能体工作区的 `agent.json`（如 `~/.aiwork/workspaces/default/agent.json`）里找到 `channels.azure_bot`，填入对应信息：
...
在代理的 `agent.json` 文件中（例如 `~/.aiwork/workspaces/default/agent.json`）找到 `channels.slack` 部分，填写相关字段：
