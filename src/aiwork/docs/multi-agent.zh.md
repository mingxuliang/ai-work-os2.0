...
**注意**：删除后工作区目录会保留（防止误删数据），如需彻底清理，请手动删除 `~/.aiwork/workspaces/{agent_id}` 目录。
...
| 环境                                       | 行为                                                                                                |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| 开启 Coding Mode + project_dir 是 git 仓库 | 在 `<project_dir>/.aiwork/worktrees/` 下创建 **git worktree**，子 Agent 在隔离的 worktree 中工作   |
| 未开启 Coding Mode + workspace 是 git 仓库 | 在 `<workspace_dir>/.aiwork/worktrees/` 下创建 **git worktree**，子 Agent 在隔离的 worktree 中工作 |
