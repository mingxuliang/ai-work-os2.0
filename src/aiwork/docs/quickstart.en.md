...
```cmd
curl -fsSL https://aiwork.agentscope.io/install.bat -o install.bat && install.bat
```
...
- For QwenPaw Local (llama.cpp), you can download models directly from the provider settings in the Console, or manually place GGUF model files in the local models directory (default `~/.aiwork/local_models/models/<org>/<model>`, for example: `~/.aiwork/local_models/models/Qwen/Qwen3-0.6B-GGUF`).
