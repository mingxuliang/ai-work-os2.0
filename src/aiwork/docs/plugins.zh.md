...
可将 `console/src/plugins/types/aiwork.d.ts` 复制到插件项目中作为 `qwenpaw-host.d.ts`，获得完整的类型提示。
...
handler 必须继承 `aiwork.runtime.commands.control.base.BaseControlCommandHandler`，并实现 `command_name`、`help_text` 和 `async handle(self, ctx, args)` 方法。
