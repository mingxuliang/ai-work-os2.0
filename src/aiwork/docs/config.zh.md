...
$QWENPAW_SECRET_DIR/                       # 默认 ~/.aiwork.secret
├── providers.json                       # 模型提供商配置与 API Key
└── envs.json                            # 环境变量
```

> **路径说明：** `$QWENPAW_WORKING_DIR` 和 `$QWENPAW_SECRET_DIR` 是环境变量，默认值分别为 `~/.qwenpaw` 和 `~/.aiwork.secret`。可通过环境变量自定义，详见下方"环境变量"章节。
...
| `QWENPAW_SECRET_DIR`       | `~/.aiwork.secret` | 敏感数据目录（存放 `providers.json` 和 `envs.json`）。Docker 中默认为 `/app/working.secret`                                                                                                                                    |
...
1. **全局配置** - `~/.aiwork/config.json`（提供商、环境变量、智能体列表）
2. **智能体配置** - `~/.aiwork/workspaces/{agent_id}/agent.json`（每个智能体的独立配置）
  ...
  "agents": {
    ...
    "profiles": {
      "default": {
        ...
        "enabled": true,
        "workspace_dir": "~/.aiwork/workspaces/default"
      }
    }
  },
  "last_api": {
...
> **模型提供商配置** 存储在 `$QWENPAW_SECRET_DIR/providers.json`（默认 `~/.aiwork.secret/providers.json`）。
> **环境变量配置** 存储在 `$QWENPAW_SECRET_DIR/envs.json`（默认 `~/.aiwork.secret/envs.json`）。
...
QwenPaw 需要 LLM 提供商才能运行。配置存储在 `$QWENPAW_SECRET_DIR/providers.json`（默认 `~/.aiwork.secret/providers.json`）。
...
每个技能是一个包含 `SKILL.md` 文件的子目录。技能的启用状态和配置存储在 `skill.json` 文件中（如 `~/.aiwork/workspaces/default/skill.json`）。
