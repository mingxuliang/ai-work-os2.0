...
my-provider (v1.0.0)
  ...
  Path: /Users/user/.aiwork/plugins/my-provider
```
...
from aiwork.plugins.api import PluginApi
import logging
...
cp -r . ~/.aiwork/plugins/my-plugin/
qwenpaw app
```

You can copy `console/src/plugins/types/aiwork.d.ts` into your plugin project as `qwenpaw-host.d.ts` for full type hints.
...
class StatusCommandPlugin:
    ...
    def register(self, api: PluginApi):
        ...
        from aiwork.runtime.commands.control.base import (
...
```bash
npm install && npm run build
cp -r . ~/.aiwork/plugins/welcome-plugin/
qwenpaw app
```
...
from aiwork.app.channels.base import (
    BaseChannel,
    OnReplySent,
    ProcessHandler,
)
...
2. View QwenPaw logs:
   ...
   ```bash
   tail -f ~/.aiwork/logs/aiwork.log | grep -i plugin
   ```
...
The handler must inherit from `aiwork.runtime.commands.control.base.BaseControlCommandHandler` and implement `command_name`, `help_text`, and `async handle(self, ctx, args)`.
...
```python
from aiwork.runtime.commands.control.base import BaseControlCommandHandler
