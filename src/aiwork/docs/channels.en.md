...
- **Edit `agent.json` directly** — Agent workspace config at `~/.aiwork/workspaces/{agent_id}/agent.json`, set `enabled: true` and fill in that platform's credentials. Saving triggers a reload without restarting the app.
...
In your agent's `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`), find `channels.dingtalk` and fill in the corresponding information, for example:
...
Find `channels.feishu` in your agent's `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`). Only **App ID** and **App Secret** are required (copy from the Feishu console under Credentials & basic info):
...
4. Set the iMessage database path. The default is `~/Library/Messages/chat.db`; use this unless you've moved the database. You can configure it in either of these ways:
   ...
   - Or edit the agent workspace `agent.json` (usually at `~/.aiwork/workspaces/default/agent.json`):
...
Find `channels.discord` in your agent's `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`) and fill in the fields, for example:
...
In your agent's `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`), find `channels.qq` and set `app_id` and `client_secret` to the values above:
...
**Method 2:** Fill in `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`)
...
| `media_dir`              | string | `~/.aiwork/media` | Media files (images, files, etc.) download directory |
| `max_reconnect_attempts` | int    | `-1`               | WebSocket max reconnect attempts (`-1` = unlimited)  |
...
- **Authentication**: On first use, scan a QR code to authorize. The token is automatically persisted to a local file (default `~/.aiwork/wechat_bot_token`), so you won't need to scan again on subsequent starts.
...
You can also configure directly in the agent workspace `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`):

```json
"wechat": {
  "enabled": true,
  "bot_token": "your_bot_token",
  "bot_token_file": "~/.aiwork/wechat_bot_token",
  "base_url": "",
  "media_dir": "~/.aiwork/media",
  "dm_policy": "open",
  "group_policy": "open"
...
| `bot_token_file` | string | `~/.aiwork/wechat_bot_token` | Path to persist the token for future runs                                             |
| `base_url`       | string | official default              | iLink API base URL; leave empty to use the official default                           |
| `media_dir`      | string | `~/.aiwork/media`            | Directory to save received images and files                                           |
...
WECHAT_BOT_TOKEN_FILE=~/.aiwork/wechat_bot_token
WECHAT_MEDIA_DIR=~/.aiwork/media
...
Find `channels.telegram` in your agent's `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`) and fill in the fields, for example:
...
Find `channels.matrix` in your agent's `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`):
...
Find `channels.azure_bot` in your agent's `agent.json` (e.g. `~/.aiwork/workspaces/default/agent.json`) and fill in:
...
Find `channels.slack` in your agent's `agent.json` (e.g., `~/.aiwork/workspaces/default/agent.json`) and fill in the fields:
...
from aiwork.app.channels.schema import ChannelType
