...
[releases]: https://aiwork.agentscope.io/downloads
...
- App logs are written to: `~/Library/Logs/io.agentscope.aiwork.desktop/qwenpaw-desktop.log`
- Backend sidecar logs live in the working directory: `~/.aiwork/desktop.log`
...
```bash
# Tail the app log
tail -f ~/Library/Logs/io.agentscope.aiwork.desktop/qwenpaw-desktop.log

# Tail the backend sidecar log
tail -f ~/.aiwork/desktop.log
```
...
1. Check the `~/.aiwork/desktop.log` file for errors
...
- **Log locations**:
  - Windows: View in the Debug shortcut terminal; app log `%LOCALAPPDATA%\io.agentscope.aiwork.desktop\logs\qwenpaw-desktop.log`; backend `%USERPROFILE%\.qwenpaw\desktop.log`
  - macOS: app log `~/Library/Logs/io.agentscope.aiwork.desktop/qwenpaw-desktop.log`; backend `~/.aiwork/desktop.log`
