...
| QwenPaw `/proactive` | `src/aiwork/agents/memory/proactive`                        | `/proactive [minutes                                                    | on                                                                                     | off]` 空闲循环 | 通过 `/api/console/chat` 发送的主动 chat request |
...
嵌入式 ReMe 配置来自 `src/aiwork/agents/memory/reme_config.py`，面向用户的默认值来自 `ReMeLightMemoryConfig`。
...
- `dream_cron_enabled` 为 true 时按 `dream_cron` 配置调度，默认 `0 23 * * *`；
...
QwenPaw 当前 `/proactive` 命令实现位于 `src/aiwork/agents/memory/proactive`，它和 ReMe 的 `proactive` job 是两套逻辑。
