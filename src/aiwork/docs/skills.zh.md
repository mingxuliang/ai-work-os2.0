...
- **技能池：** 共享本地仓库，路径是 `$QWENPAW_WORKING_DIR/skill_pool/`
  （默认 `~/.aiwork/skill_pool/`）。
- **工作区技能副本：** 某个工作区真正运行时使用的本地副本，路径是
  `$QWENPAW_WORKING_DIR/workspaces/{agent_id}/skills/`
  （默认 `~/.aiwork/workspaces/{agent_id}/skills/`）。
...
对于 `requires` 元数据，解析器按顺序检查：`metadata.openclaw.requires` → `metadata.aiwork.requires` → `metadata.requires`，取第一个找到的。
