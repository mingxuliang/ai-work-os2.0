...
**默认保护**: `{WORKING_DIR}.secret/` 目录(存储 API 密钥、认证凭据和提供商配置)默认包含在敏感文件列表中。默认情况下,`WORKING_DIR` 为 `~/.aiwork/`,完整路径为 `~/.aiwork.secret/`。
...
扫描器使用 `src/aiwork/security/skill_scanner/rules/signatures/` 中的 YAML 规则文件。你可以通过 YAML 策略文件自定义扫描策略:
...
```bash
# 删除认证文件
rm ~/.aiwork.secret/auth.json  # 或 $WORKING_DIR.secret/auth.json
