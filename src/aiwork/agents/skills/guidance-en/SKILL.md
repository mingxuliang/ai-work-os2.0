...
```bash
DOCS_DIR=$(python3 -c "from aiwork.constant import DOCS_DIR; print(DOCS_DIR or '')" 2>/dev/null)
```
...
DOCS_DIR=$(find ~/.aiwork/memory/ -type d -name "docs")
```
...
# Logical deduction: if the path contains .aiwork/bin/qwenpaw, the root is three levels up
# Example: /path/to/QwenPaw/.aiwork/bin/qwenpaw -> /path/to/QwenPaw
if [[ "$QWENPAW_PATH" == *".aiwork/bin/qwenpaw" ]]; then
    QWENPAW_ROOT=$(echo "$QWENPAW_PATH" | sed 's/\/\.qwenpaw\/bin\/aiwork//')
else
