...
Path: `~/.aiwork/config.json`, under `channels.dingtalk`.
...
2. The agent must not proactively modify the `console` configuration or `~/.aiwork/config.json`.
3. You must instruct the user to fill in the credentials manually using one of the following two methods:
   ...
   - Configuration file: edit the `channels.dingtalk` field in `~/.aiwork/config.json`
