...
from aiwork.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from aiwork.tauri.sidecar_logging import install_sidecar_logging
...
def _ensure_qwenpaw_app_not_loaded() -> None:
    if "aiwork.app._app" in sys.modules:
        raise RuntimeError(
...
def _sync_loaded_qwenpaw_constant_cors_origins() -> None:
    constant_module = sys.modules.get("aiwork.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from aiwork.constant.CORS_ORIGINS at import time.
...
def _run_backend_server(log_level: str) -> None:
    ...
    from aiwork.config.utils import write_last_api
    from aiwork.constant import LOG_LEVEL_ENV, WORKING_DIR
    from aiwork.utils.logging import (
        SuppressPathAccessLogFilter,
        setup_logger,
    )
    from aiwork.utils.port import get_stable_port, write_port_file
    ...
    from aiwork.tauri.backend_guard import reconcile_singleton_backend

    reconcile_singleton_backend(WORKING_DIR)
    if normalized_log_level in ("debug", "trace"):
        from aiwork.cli.main import log_init_timings
...
def main() -> None:
    ...
    if not config_path.exists():
        from aiwork.cli.init_cmd import init_cmd
