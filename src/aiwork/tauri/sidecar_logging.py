...
def install_sidecar_logging(log_path: Path) -> Path:
    ...
    logging.getLogger("aiwork.tauri").info(
...
def _add_project_file_handler(log_path: Path) -> None:
    # Import lazily so aiwork.constant is loaded only after desktop env setup.
    from aiwork.utils.logging import add_project_file_handler
