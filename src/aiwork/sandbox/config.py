...
This is the natural entry point into the :mod:`aiwork.sandbox` package:
...
Backend layout (``create_sandbox`` dispatches to these by ``SandboxMode``):
  - SEATBELT     → mod:`aiwork.sandbox.macos_sandbox`      (MacOSSandbox)
  - BUBBLEWRAP   → mod:`aiwork.sandbox.bubblewrap_sandbox` (BubblewrapSandbox)
  - LANDLOCK     → mod:`aiwork.sandbox.linux_sandbox`      (LinuxSandbox)
  - APPCONTAINER → Windows sandbox, dispatched on ``allow_read_all``:
      True  → windows_restricted_sandbox (WindowsRestrictedSandbox)
      False → mod:`aiwork.sandbox.windows_sandbox`            (WindowsSandbox)
  - NONE         → mod:`aiwork.sandbox.local_sandbox`      (NoneSandbox)

Shared base class for all backends:
  - :class:`aiwork.sandbox.local_sandbox.LocalSandbox`
